...
func TestParseScenariosCSV(t *testing.T) {
	...
	scenariosHeaderLine := []string{"name", "type", "credentialLocation", "from", "to", "description"}
	createScenariosHeadersMap, err := CreateScenariosHeadersMap(scenariosHeaderLine)
	if err != nil {
		errorString := FormatTestFailureString("Create Header Map", err, "map[0:name 1:type 2:credentialLocation 3:from 4:to 5:description]")
		t.Error(errorString)
	}

	// Happy Path: Validate scenario line with empty description field.
	...
	scenariosTestLineTwo := []string{"internal-external", "", "database", "a@a.com", "b@b.com", "Mail from platform users"}
	...
	scenariosTestLineThree := []string{"internal-external", "OF365", "database", "a", "b@b.com", "Mail from platform users"}
	...
	scenariosTestLineFour := []string{"internal-external", "OF365", "", "a@a.com", "b@b.com", "Mail from platform users"}
